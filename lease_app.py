...
def run_ccr_balancing_loop(target_loop_amount, msrp, lease_cash_applied, residual_value, term_months, mf, county_tax, q_value, tolerance=0.005, max_iterations=1000):
    min_ccr = 0.0
    max_ccr = msrp - residual_value
    iteration = 0

    cap_cost = msrp + 900  # Include Doc + Acq fees in Cap Cost
    ...
    while iteration < max_iterations:
        ...
        base_payment_loop_rounded = round(base_payment_loop_exact, 2)
        monthly_tax_loop = round(base_payment_loop_rounded * county_tax, 2)

        ltr_fee_tax = round(q_value * county_tax, 2)

        first_month_payment = round(base_payment_loop_rounded + monthly_tax_loop + q_value + ltr_fee_tax, 2)
