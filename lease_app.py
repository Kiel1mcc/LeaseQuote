...
if vin_input:
    ...
    if vin_data.empty:
        st.warning("Vehicle not found in inventory. Enter manually:")
        manual_entry = True
        model_number = st.text_input("Model Number")
        msrp = st.number_input("MSRP ($)", min_value=0.0, value=25000.0, step=100.0)
...
# Proceed if we have a model number
if vin_input and model_number:
