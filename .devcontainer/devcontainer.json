...
def process_lease_data():
    # Diagnostic setup within function
    st.write("Processing lease data. Current directory:", os.getcwd())
    st.write("Python path:", sys.path)
    st.write("Files in current directory:", os.listdir(os.getcwd()))

    # Explicit import with error handling
    try:
        # Check if the file exists
        calc_file = os.path.join(os.getcwd(), "lease_calculations.py")
        if not os.path.isfile(calc_file):
            st.error(f"File not found: {calc_file}")
            st.stop()
        
        # Load the module explicitly
        spec = importlib.util.spec_from_file_location("lease_calculations", calc_file)
        lease_calculations = importlib.util.module_from_spec(spec)
        spec.loader.exec_module(lease_calculations)
        calculate_base_and_monthly_payment = lease_calculations.calculate_base_and_monthly_payment
        st.write("Import successful using explicit path")
        ...
        st.write("Module load attempt failed. Check for circular imports or file issues.")
        st.stop()
    except AttributeError:
        st.error("AttributeError: 'calculate_base_and_monthly_payment' not found in lease_calculations.py")
        st.stop()
